...
class QFormer(nn.Module):
    ...
    def forward(self, image_features, text_features=None, text_attention_mask=None):
        """
        Forward method supporting image-text interaction.

        Args:
            image_features (torch.Tensor): Vision features [batch_size, num_patches, hidden_size]
            text_features (torch.Tensor): Text features [batch_size, seq_len, hidden_size]
            text_attention_mask (torch.Tensor): Attention mask for text [batch_size, seq_len]

        Returns:
            torch.Tensor: Joint image-text embeddings
        """
        device = image_features.device

        # Image attention mask
        image_attention_mask = torch.ones(image_features.size()[:-1], dtype=torch.long).to(device)

        # Expand query tokens for the batch
        query_tokens = self.query_tokens.expand(image_features.shape[0], -1, -1).to(device)

        # Combine image and text features for cross-attention
        if text_features is not None:
            # Concatenate image and text features
            combined_features = torch.cat([image_features, text_features],
                                          dim=1)  # [batch_size, num_patches + seq_len, hidden_size]

            # Combine attention masks
            combined_attention_mask = torch.cat([image_attention_mask, text_attention_mask],
                                                dim=1)  # [batch_size, num_patches + seq_len]
        else:
            combined_features = image_features
            combined_attention_mask = image_attention_mask

        # Pass through QFormer with combined features
        ...
        # Extract and project final embeddings
        joint_embeddings = query_output.last_hidden_state
        joint_embeddings = self.projector(joint_embeddings)

        return joint_embeddings

 

@register_connector('q_former')
