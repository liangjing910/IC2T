...
from .connector.q_former import QFormer
...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    def __init__(self, config: TinyLlavaConfig):
        ...
        # self.connector = QFormer(config)
        # 初始化独立的 Q-Former 实例
        self.qformer_round1 = QFormer(config)  # 用于第一轮任务
        self.qformer_round2 = QFormer(config)  # 用于第二轮任务
        self.post_init()
    ...
    def encode_images(self, images):
        ...
        image_features = self.vision_tower(images, **kwargs)#(1, 728, 1152)
        image_features = self.connector(image_features)
        return image_features

    def encode_images_with_text(self, images, input_ids, round_number):
        """
        编码图像和文本特征，并将其结合处理。

        Args:
            images (torch.Tensor): 输入图像张量，形状为 [batch_size, 3, height, width]。
            input_ids (torch.Tensor): 文本输入 token IDs，形状为 [batch_size, seq_len]。
            round_number (int): 当前任务轮次，1 或 2。

        Returns:
            torch.Tensor: 图文交互后的特征。
        """
        kwargs = {}
        kwargs['vision_feature_layer'] = self.config.vision_feature_layer
        kwargs['vision_feature_select_strategy'] = self.config.vision_feature_select_strategy
        ...
        # 图像特征提取
        images = images.to(device=self.device, dtype=self.dtype)
        image_features = self.vision_tower(images, **kwargs)  # 输出形状 [batch_size, num_patches, 1152]

        # 文本特征提取
        input_ids = input_ids.to(device=self.device)
        text_features = self.language_model(input_ids,
                                            return_dict=True).last_hidden_state  # 输出形状 [batch_size, seq_len, hidden_size]

        # 根据轮次调用对应的 Q-Former
        if round_number == 1:
            multimodal_features = self.qformer_round1(image_features, text_features)  # 调用 Q-Former 1
        elif round_number == 2:
            multimodal_features = self.qformer_round2(image_features, text_features)  # 调用 Q-Former 2
        else:
            raise ValueError(f"Invalid round_number: {round_number}. It must be 1 or 2.")
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels
        if images.ndim == 5:
            # 检查 input_ids 的长度是否足够拆分
            num_images = images.shape[1]
            # 根据 num_images 动态设置 round_number
            if num_images == 2:
                round_number = 1
            elif num_images == 4:
                round_number = 2
            image_positions = (input_ids == IMAGE_TOKEN_INDEX).nonzero(as_tuple=True)[1]  # 获取占位符位置
            if len(image_positions) != num_images:
                raise ValueError(
                    f"Number of image placeholders ({len(image_positions)}) does not match number of images ({num_images})."
                )
            # 按占位符拆分 input_ids
            split_input_ids = []
            prev_pos = 0
            for pos in image_positions:
                split_input_ids.append(input_ids[:, prev_pos:pos])
                prev_pos = pos + 1  # 跳过占位符
            split_input_ids.append(input_ids[:, prev_pos:])  # 处理最后一段文本
            # 确保分割后数量与图片数量一致
            if len(split_input_ids) != num_images:
                raise ValueError("Mismatch between number of split text segments and number of images.")

            # 分别处理每张图片及对应的文本
            image_features = torch.stack(
                [self.encode_images_with_text(img, text_ids, round_number) for img, text_ids in zip(images, split_input_ids)], dim=0
            )
        else:
            image_features = self.encode_images_with_text(images, input_ids, round_number)
